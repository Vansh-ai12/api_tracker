...
export async function GET() {
  ...
  const { data: user, error } = await supabase
    .from("users")
    .select("tracking_mode, gmail_connected, gmail_email, gmail_connected_at, gmail_last_scan_at, gmail_last_scan_status, gmail_last_error, forwarding_alias")
    .eq("id", userId)
    .maybeSingle();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "disconnect") {
      ...
      logAuditEvent("gmail_disconnected", { userId, telegramChatId: user.telegram_chat_id });
