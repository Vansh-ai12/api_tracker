...
async function getSubscriptions(userId: string): Promise<Subscription[]> {
  ...
  const { data: subscriptions, error: subError } = await supabase
    .from("subscriptions")
    .select(
      "id, service_name, amount, currency, billing_cycle, renewal_date, status, created_at",
    )
    .eq("user_id", userId)
...
async function getTelegramConnection(userId: string): Promise<boolean> {
  ...
  return (
    data?.telegram_chat_id !== null && data?.telegram_chat_id !== undefined
  );
}
...
async function getUserGmailStatus(userId: string) {
  ...
  const { data } = await supabase
    .from("users")
    .select(
      "tracking_mode, gmail_connected, gmail_email, gmail_last_scan_at, gmail_last_scan_status, gmail_last_error, forwarding_alias",
    )
    .eq("id", userId)
    .maybeSingle();
  ...
  return {
    tracking_mode:
      (data?.tracking_mode as "GMAIL" | "PRIVATE_EMAIL") || "PRIVATE_EMAIL",
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full mr-1.5 ${isActive ? "bg-emerald-500 animate-pulse" : "bg-gray-400"}`}
      />
...
function SubscriptionCard({ sub }: { sub: Subscription }) {
  ...
  const initialLetter = sub.service_name
    ...
    <div className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-100 dark:border-gray-800/80 p-5 flex flex-col justify-between gap-4 shadow-sm hover:shadow-md dark:hover:border-gray-700 transition-all group">
      ...
      <div className="grid grid-cols-2 gap-4 pt-3 border-t border-gray-50 dark:border-gray-800/50 text-sm">
        <div>
          ...
          <p className="font-bold text-[#0a0a0a] dark:text-gray-100 text-base">
            {amountLabel}
          </p>
        </div>
        <div>
          ...
          <p className="font-semibold text-[#0a0a0a] dark:text-gray-200">
            {renewalLabel}
          </p>
        </div>
      </div>
...
export default async function DashboardPage() {
  ...
  const totalMonthlySpend = active.reduce((acc, sub) => {
    ...
    if (sub.billing_cycle === "yearly")
      return acc + Math.round(sub.amount / 12);
    return acc + sub.amount;
  }, 0);

  // Determine nearest upcoming renewal
  const nextRenewal =
    active.length > 0 && active[0].renewal_date
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-2 max-w-2xl">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">
                  Free accounts track receipts & spending statistics. Upgrade to{" "}
                  <strong>Unsub Pro for ₹49/month</strong> to unlock automatic
                  Telegram nudges & browser notifications before your credit
                  card gets charged.
                </p>
              </div>
          ...
          {subscriptions.length === 0 ? (
            ...
            <div className="text-center py-16 px-6 bg-white dark:bg-[#141414] rounded-3xl border border-gray-100 dark:border-gray-800/80 shadow-xs">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto leading-relaxed mb-6">
                Forward your subscription receipts (Netflix, Spotify, ChatGPT,
                Canva, etc.) to your unique email address below to track them
                automatically:
              </p>
